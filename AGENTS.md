...
- `packages/auth/src/waitlist.ts`: `createWaitlist`: an `InitiateGate` that pre-creates PENDING users for unknown identifiers, redirects them to the app's waitlist page, and lets the app's `autoApprove` hook skip it; statuses live behind the app's `ApprovalStore`. `src/waitlist-email.ts` renders the admin notification (`waitlistNotificationEmail`); the app sends it.
...
- `packages/auth-store-prisma/src/prisma-identity-store.ts` — `createPrismaIdentityStore`, a Prisma-backed `IdentityStore`. Types the model delegate structurally (`PrismaIdentityModel`) so the package never imports `@prisma/client`. `UserStore` stays app-side.
- `packages/auth/src/admin/admin-data.ts` — framework-agnostic admin dashboard data (`createAdminData`), exported at the `@activescott/auth/admin` subpath (barrel: `src/admin/index.ts`). Reads `UserStore.listUsers` / `IdentityStore.findByUserIds` (both optional) and `Auth.describeConfig()`. Never copies `Identity.providerState` into its output.
- `packages/auth/src/admin/admin-predicate.ts` — the admin allowlist check (`createAdminPredicate`, `isAdminUser`, `AdminPredicate`, `ADMIN_IDENTIFIERS_ENV`), same subpath. Matches every identity a user owns, not just the session's, and fails closed on an empty allowlist. The react-router adapter's admin gate re-exports it rather than keeping its own copy.
- `packages/auth-adapter-react-router/src/handlers.ts` — `createAuthHandlers` and friends.
- `packages/auth-adapter-react-router/src/page-loaders.ts`: `createAuthPageLoaders` (`signInLoader`, `profileAuthLoader`), the server half of sign-in and profile pages. The browser half is `src/client/`, the `./client` subpath: headless hooks (`useTurnstile`, passkey, OTP autofill, preserved input) that import React but nothing from `react-router`, and take the passkey client as a parameter so the adapter never depends on the passkey package.
- `packages/auth-adapter-react-router/src/profile/` — the profile page as components (`ProfilePage`, `AccountSummary`, `SignInMethods`, `Passkeys`), at the `./profile` subpath. Same shape as `src/admin/`: React but nothing from `react-router`, a `classNames` slot map over built-in inline styles (`profile-styles.ts`), links through `linkComponent`.
