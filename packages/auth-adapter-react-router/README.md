...
The comparison is against `request.url`, so behind a proxy that terminates TLS the app still has to trust the proxy (`app.set("trust proxy", 1)` in express), or every post from the browser looks cross-origin.
